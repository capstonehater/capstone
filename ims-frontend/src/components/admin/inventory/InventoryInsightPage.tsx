        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 p-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {kind === "value" ? <label className="flex items-center gap-2 text-sm font-medium">Sort by<select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="rounded-lg border border-slate-300 bg-white px-3 py-2"><option value="amount">Inventory Value</option><option value="quantity">Usable Stock</option></select></label> : null}
              <label className="flex items-center gap-2 text-sm font-medium">{kind === "value" ? "Order" : `Sort by ${config.amount.toLowerCase()}`}<select value={direction} onChange={(event) => setDirection(event.target.value)} className="rounded-lg border border-slate-300 bg-white px-3 py-2"><option value="asc">Ascending (lowest first)</option><option value="desc">Descending (highest first)</option></select></label>
